...
class AuthItemGroupSearch extends AuthItemGroup
{
    ...
    public function search($params)
    {
        ...
        if ($this->created_at) {
            ...
            if (isset($tmp[0], $tmp[1])) {
                $query->andFilterWhere([
                    'between',
                    Yii::$app->getModule('user-management')->auth_item_group_table . '.created_at',
                    strtotime($tmp[0]),
                    strtotime($tmp[1])
                ]);
            }
        ...
        $query->andFilterWhere([
            'like',
            Yii::$app->getModule('user-management')->auth_item_group_table . '.code',
            $this->code
        ])
            ->andFilterWhere([
                'like',
                Yii::$app->getModule('user-management')->auth_item_group_table . '.name',
                $this->name
            ]);
