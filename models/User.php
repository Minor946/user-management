...
namespace minor946\modules\UserManagement\models;

use minor946\modules\UserManagement\components\AuthHelper;
use minor946\modules\UserManagement\components\UserIdentity;
...
class User extends UserIdentity
{
    ...
    public static function revokeRole($userId, $roleName)
    {
        $result = Yii::$app->db->createCommand()
                ->delete(Yii::$app->getModule('user-management')->auth_assignment_table,
            ...
            foreach ($ips as $ip) {
                if (!filter_var(trim($ip), FILTER_VALIDATE_IP)) {
                    $this->addError('bind_to_ip',
                        UserManagementModule::t('back', "Wrong format. Enter valid IPs separated by comma"));
                }
            }
        }
    }
